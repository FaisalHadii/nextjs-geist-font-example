...
PATH_TO_OUTPUT = OUTPUT_DIR  # Use the same output directory
    ...
    try:
        ...
        with uproot.open(root_file) as file:
            ...
            if tree_name in file:
                ...
                if "Ekine" in tree.keys() and "dX" in tree.keys() and "dY" in tree.keys() and "dZ" in tree.keys() and total_count > 0:
                    print(f"  Filtering for energy {min_E:.3f}-{max_E:.3f} MeV and zero scatter angle...")
                    energies = tree["Ekine"].array(library="np")
                    dx = tree["dX"].array(library="np")
                    dy = tree["dY"].array(library="np")
                    dz = tree["dZ"].array(library="np")
                    
                    # Calculate scatter angles
                    scatter_angles = np.arctan(np.sqrt(np.power(dx, 2) + np.power(dy, 2)) / dz) * 180 / np.pi
                    
                    # Filter for both energy and zero scatter angle (with small tolerance)
                    angle_tolerance = 0.001  # Allow angles within ±0.001 degrees
                    mask = (energies >= min_E) & (energies <= max_E) & (np.abs(scatter_angles) <= angle_tolerance)
                    filtered_count = np.sum(mask)
                    print(f"  Found {filtered_count} particles within energy window and zero scatter angle.")
                elif "Ekine" not in tree.keys():
                    print(f"  Error: Required branches not found in TTree '{tree_name}'. Cannot filter particles.")
                    filtered_count = None
                else:
                    filtered_count = 0
                    print(f"  Found {filtered_count} particles within energy window and zero scatter angle.")
...
ax.set_title('Attenuation Analysis (Zero Scatter Angle Only)', fontsize=16, fontweight='bold')
...
# --- Scatter Angle Analysis ---
print("\nAnalyzing particle scatter angles...")

try:
    with uproot.open(PATH_TO_OUTPUT + "phantom_transmission_TEMPLATE.root") as rootfile:
        data = rootfile['PhaseSpace'].arrays(filter_name=["*"], library='np')

        # Filter particles according to their energy (150 keV +- 0.001)
        filter_indices = np.where((data['Ekine'] >= 0.149999) & (data['Ekine'] <= 0.150001))

        # Calculate incident angles
        incident_angles = np.arctan(np.sqrt(np.power(data['dX'][filter_indices], 2) + 
                                          np.power(data['dY'][filter_indices], 2)) / 
                                  data['dZ'][filter_indices])

        # Convert to degrees
        incident_angles = incident_angles * 180 / np.pi

        # Create histogram with fine resolution (0.001°) between -1° and 1°
        angleBinEdges = np.arange(-1, 1 + 0.001, 0.001)  # From -1° to 1° with 0.001° steps
        histData, histBins = np.histogram(incident_angles, bins=angleBinEdges)
        angleBinCentres = 0.5 * (angleBinEdges[:-1] + angleBinEdges[1:])

        # Create new figure for scatter angle plot with modern styling
        plt.style.use('classic')  # Use classic style which is always available
        fig_scatter, ax_scatter = plt.subplots(figsize=(10, 6))
        
        # Plot with enhanced styling
        ax_scatter.plot(angleBinCentres, histData, linewidth=1.5, color='#1f77b4')
        ax_scatter.set_yscale('log')
        ax_scatter.set_xlabel('Scatter Angle [°]', fontsize=14)
        ax_scatter.set_ylabel('Frequency', fontsize=14)
        ax_scatter.set_xlim(-1, 1)  # Set x-axis limits to show only -1° to 1°
        ax_scatter.set_title('Particle Scatter Angle Distribution (-1° to 1°)', 
                           fontsize=16, fontweight='bold')
        
        # Enhanced grid and styling
        ax_scatter.grid(True, linestyle='--', linewidth=0.8, alpha=0.7)
        ax_scatter.set_facecolor('#f5f5f5')
        ax_scatter.tick_params(axis='both', which='major', labelsize=12)

        plt.tight_layout()
        plt.show()
        plt.pause(0.001)  # Ensure plot is shown in non-interactive environment

except Exception as e:
    print(f"Error in scatter angle analysis (-1° to 1° range): {e}")
    print("Check if the ROOT file contains the expected data for the given energy filter.")
    print("Continuing with rest of the script...")
